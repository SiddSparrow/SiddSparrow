# Hi, I'm Fábio Nascimento

**Full-Stack Software Engineer** based in Rio de Janeiro, Brazil (GMT-3).

I build scalable SaaS applications, RESTful APIs, and modern web interfaces with **Laravel**, **React/Next.js**, **Vue.js**, and **TypeScript**. I have 6+ years of experience shipping production applications — from database design and API architecture to responsive frontends and cloud deployment.

Before transitioning into tech, I spent 10 years as an educator, which shaped how I approach documentation, mentoring, and cross-functional communication.

---

###  Tech Stack

**Frontend:** React · Next.js · Vue.js · TypeScript · Tailwind CSS · Framer Motion  
**Backend:** PHP (Laravel 7–12) · Node.js · Express · REST API Design  
**Databases:** PostgreSQL · MySQL · Redis · Supabase  
**DevOps:** Docker · AWS · Nginx · Vercel · GitHub Actions · Linux  
**Integrations:** Anthropic Claude API · OpenAI API · Payment Gateways · Webhooks  
**Testing:** PHPUnit · Jest · React Testing Library · Cypress

---

###  Featured Projects

| Project | Stack | Description |
|---------|-------|-------------|
| [**TestMaker**](https://github.com/SiddSparrow/TestMaker) | Laravel 12, Vue.js 3, PostgreSQL, Claude API | AI-powered SaaS for exam creation with intelligent question generation, PDF/DOCX export, and multi-step configuration UI |
| [**SuperSDR**](https://github.com/SiddSparrow/super-sdr-avaliacao) | TypeScript, Express, PostgreSQL, Claude API | Webhook normalization system with async processing, API documentation via OpenAPI/Swagger |
| [**Multi-Tenant CRM**](https://github.com/SiddSparrow/sdr-crm-prova) | Next.js, React, Supabase, TypeScript | CRM with Row Level Security, AI-powered messaging, and integrated auth — built in 72 hours |
| [**Email Classifier**]([https://github.com/SiddSparrow/sdr-crm-prova](https://github.com/SiddSparrow/personal_email_classifier)) | Python, NLP, Gmail API, Telegram API | Email Classifier that Fetches Emails from Gmail and Sends Notifications via Telegram |

<!--| [**Landing Pages**](https://github.com/SiddSparrow/barbershop-landing) | Next.js, Tailwind CSS, Framer Motion, Sanity CMS | Reusable, responsive landing page templates with headless CMS and Vercel deployment | -->

---

###  Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/fabioduqueestrada)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:fabio.duqueestrada@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/SiddSparrow)

---

*Currently open to Full-Stack and Backend opportunities — remote or hybrid.*
